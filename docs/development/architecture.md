...
<!-- doc-review {"version":1,"fingerprint":"ea361789d44efe207bf06135f53ab0dc83a280dec5946e6913ecd4275d201ade","dependencies":"docs/development/.reviews/architecture/trace-an-edit.json","dependencyDigest":"542b328bc70ae51ff7000e53dd7ace00d375825cf1b59eda6ff0d762dad74462","disposition":"still accurate","rationale":"Unchanged prose. The changed dependency is src/presentation/surface-controls.mjs, and this section's claim about it is that surface controls, their placement lifecycle and mirror controls keep previews outside authored state. That is still exactly true: the ghost's stippled shadow, its drop-line, the landing ring and the gap chip are preview drawing that writes no authored state and sends no command, and read() gained observational fields only, with no new writer. The merged strip's change to the same file routes the panel's state word and confirming control name through the shared placement vocabulary, which moves no ownership either. The new import of BUILD_ENVIRONMENT is presentation reading model, an allowed edge. The traced edit path and its layer ownership are unaffected."} -->
