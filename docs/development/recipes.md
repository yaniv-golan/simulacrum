...
<!-- doc-review {"version":1,"fingerprint":"21b88cde4132d26aacf5976e52f1ab573e794874c85ce180cdedd4021a78f236","dependencies":"docs/development/.reviews/recipes/change-an-interaction.json","dependencyDigest":"025310c56f319583b05238e148d1cb2722caa3e94230f5a82eabb1b22348ab66","disposition":"still accurate","rationale":"Hosted-ready check waits (claude/hosted-check-waits on 458e9b3c, stacked on q44-release-reuse): the shared browser session now scales every page deadline by the profile's registered `waitScale` (clamped to 60 % of the row budget), budgets workshop startup separately from the interaction deadline, records `waitScale`/`liveWaits`/`targetGeometry` in its evidence, and refuses a wait scale that did not come from the suite; `liveWait` takes its slice from `liveSliceMs` and samples frame gaps; `host-profile.mjs` registers the two facts and passes them to check children as numbers. verify-mirror-browser reloads through `evidence.reload` (startup budget) and part-help-cases routes one literal through `waitBudget`; the recipe's interaction-change steps, owners and evidence requirements are unchanged."} -->
...
<!-- doc-review {"version":1,"fingerprint":"7ccac1effee8cfe31a20f233d8e7a7b445e1192ef01e36d1bf2858235a7876a2","dependencies":"docs/development/.reviews/recipes/change-physics.json","dependencyDigest":"a51ac0b4e7669080488a69dcbadfc5534422ff60de39e403e01e46f3bc3640b5","disposition":"still accurate","rationale":"--stack + item 52 stacked on claude/hosted-check-waits 94c85e92 (tooling-stack-flag, base 458e9b3c): re-recorded after merging webmcp's hosted-check-waits tip and folding item 52 (candidate-after requires a structural check under the gate's `structural:` receipt key; fixtures renamed) — --stack stacked on the frozen chain (tooling-stack-flag merged with q44-release-reuse 434fe72c, base 458e9b3c): re-recorded after merging the chain (release-identity's source record, item 50's refused-row marking, feedback-sync, q44's release reuse) — Stacked candidates by --stack (tooling-stack-flag on main 458e9b3c): the drift is verification tooling — a derived stacking flag, its parser, window intent, tests and documentation; no simulation, physics-door, power, checkpoint or contact behavior changed."} -->
...
<!-- doc-review {"version":1,"fingerprint":"4285feabaed7aebb0173fcc59ed538940103e220e3f1912db095ffacbc94c54c","dependencies":"docs/development/.reviews/recipes/change-multi-part-authoring.json","dependencyDigest":"2a25dbd6d1cb28a3121507f8004c99e8331b172a563811d08d17ca7384e798f1","disposition":"still accurate","rationale":"Hosted-ready check waits (claude/hosted-check-waits on 458e9b3c, stacked on q44-release-reuse): the shared browser session now scales every page deadline by the profile's registered `waitScale` (clamped to 60 % of the row budget), budgets workshop startup separately from the interaction deadline, records `waitScale`/`liveWaits`/`targetGeometry` in its evidence, and refuses a wait scale that did not come from the suite; `liveWait` takes its slice from `liveSliceMs` and samples frame gaps; `host-profile.mjs` registers the two facts and passes them to check children as numbers. verify-assemblies-browser reloads through `evidence.reload` and assembly-ux-cases routes one literal; the authoring recipe's steps and its browser evidence requirements are unchanged."} -->
...
<!-- doc-review {"version":1,"fingerprint":"57727b6017574f68a193de5382ccfe9d022d8d71dc5f860aa933670f427457f1","dependencies":"docs/development/.reviews/recipes/change-a-presentation-overlay.json","dependencyDigest":"878b1f4e33373dbf48345be8af5abf76a6271c0a8d5b397954b8395a876e9802","disposition":"still accurate","rationale":"Hosted-ready check waits (claude/hosted-check-waits on 458e9b3c, stacked on q44-release-reuse): the shared browser session now scales every page deadline by the profile's registered `waitScale` (clamped to 60 % of the row budget), budgets workshop startup separately from the interaction deadline, records `waitScale`/`liveWaits`/`targetGeometry` in its evidence, and refuses a wait scale that did not come from the suite; `liveWait` takes its slice from `liveSliceMs` and samples frame gaps; `host-profile.mjs` registers the two facts and passes them to check children as numbers. verify-rope-browser (build-backlog's rewrite) builds its repaired rig supported on cell stacks, waits with `liveWait`, and bounds the repair horizon by ticks via `advanceTime` — a check-meaning fix, not an overlay change; the recipe's overlay steps and evidence are unchanged."} -->
