...
<!-- doc-review {"version":1,"fingerprint":"c8499ddb88029f792dcbdb07b6eef807ccd5a434f47099e99a2acf5d88c0bcd2","dependencies":"docs/development/.reviews/recipes/change-physics.json","dependencyDigest":"66f3e53ac9a40a43236cdbddc0505362831786308c038a037657f523b7f53348","disposition":"still accurate","rationale":"Hosted-ready check waits (claude/hosted-check-waits on 458e9b3c, stacked on q44-release-reuse): the shared browser session now scales every page deadline by the profile's registered `waitScale` (clamped to 60 % of the row budget), budgets workshop startup separately from the interaction deadline, records `waitScale`/`liveWaits`/`targetGeometry` in its evidence, and refuses a wait scale that did not come from the suite; `liveWait` takes its slice from `liveSliceMs` and samples frame gaps; `host-profile.mjs` registers the two facts and passes them to check children as numbers. The changed dependencies reach this section only through its evidence and harness scripts (check scripts' waits, the format gate's colour-safe parser, the manifest profile facts, the README paragraph); no physics law, fixture, evidence assertion, contact or reset policy changed."} -->
...
<!-- doc-review {"version":1,"fingerprint":"4267710548dafbb8fae3407c3cc0c5d613bfac6a61b5fb69fcca31e830bd92db","dependencies":"docs/development/.reviews/recipes/change-a-presentation-overlay.json","dependencyDigest":"0dc4a315dbf3a18951f2df54834905308b52731fafcf6f0ccd1b2c0528ee1da2","disposition":"still accurate","rationale":"Hosted-ready check waits (claude/hosted-check-waits on 458e9b3c, stacked on q44-release-reuse): the shared browser session now scales every page deadline by the profile's registered `waitScale` (clamped to 60 % of the row budget), budgets workshop startup separately from the interaction deadline, records `waitScale`/`liveWaits`/`targetGeometry` in its evidence, and refuses a wait scale that did not come from the suite; `liveWait` takes its slice from `liveSliceMs` and samples frame gaps; `host-profile.mjs` registers the two facts and passes them to check children as numbers. verify-rope-browser (build-backlog's rewrite) builds its repaired rig supported on cell stacks, waits with `liveWait`, and bounds the repair horizon by ticks via `advanceTime` — a check-meaning fix, not an overlay change; the recipe's overlay steps and evidence are unchanged."} -->
