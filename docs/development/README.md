...
## Milestone status
<!-- doc-review {"version":1,"fingerprint":"43f0edf02f860817dc1a90aa83f6316cec69459ecca89e46428168f1c3c2ddf6","dependencies":"docs/development/.reviews/README/milestone-status.json","dependencyDigest":"73f32d57a46e6ddfe22f7c6bf6b677f7d421c89470bae922fb804f13ea9857df","disposition":"still accurate","rationale":"Unchanged prose. The only changed dependency is scripts/manifest.json, and its whole diff is 15 consumerSourceHash values in browserReviewMetadataScopes (15 insertions, 15 deletions, no other key). The milestone field is still M3b; no milestone allocation, bar contract, invariant or rule/check ownership row moved, and no browserChecks row changed at all. The version bump to 0.5.0 and the save-format release note add no part, feature or milestone breadth, so this section's statement of the current construction loop and the rule that the manifest owns allocation both still hold."} -->
...
<!-- doc-review {"version":1,"fingerprint":"3a7f5f5636f514cd68d381a351880a298ca9990cfcd7932538d5ff36555a62d9","dependencies":"docs/development/.reviews/README/verify-a-change.json","dependencyDigest":"eab7530c8f12876ed712ba1764c1ad027da874f444ebb92a22ac9c497e7075bb","disposition":"still accurate","rationale":"Unchanged prose. The only changed dependency is package.json, and its only edit is the version field, 0.4.0 to 0.5.0; no npm script was added, removed or retargeted, so every command name this section lists still resolves to the same script. The tier definitions, the local/merge/final distinction, the verification-outcome exit codes, the human-session rules and the waiver-still-enumerates rule are all independent of the version string."} -->
...
<!-- doc-review {"version":1,"fingerprint":"e448ca9ccb83ecd04dd1d537a4adfd4c42557db9e3a2516ec01c02da5dc582ed","dependencies":"docs/development/.reviews/README/browser-execution-and-scope.json","dependencyDigest":"40228fd3adc0b25d2e5be80fd20975354a915347714ff5fa0335e927c4712768","disposition":"still accurate","rationale":"Unchanged prose. No browser check was registered, removed or re-scoped: every browserChecks row, every browserLocalScopes row and every other field of every browserReviewMetadataScopes row (entrypoint, sourceSha256, reads, purposes, both excluded inputs, dependencies, consumers, reaching checks) is byte-identical, so no timing budget was widened. The manifest's whole diff is 15 consumerSourceHash values, applied through browser:scopes prepare --base 619a4675 and apply with a per-row rationale; each of those rows' hashed closure contains src/application/release-notes.mjs or the regenerated src/model/generated/blueprint-validator.mjs, or both, which is exactly the reachable-consumer-set-expanded-by-dependencies behavior this section describes, and every proposed hash was re-derived independently from inspectScopeInputs' own graph and reads before acceptance. The apply's invariant-controls witnesses passed and its affected-but-not-witnessed checks are enumerated NOT_EXECUTED rather than claimed. Selection, phase scheduling, host profiles, registered platforms and the measured-scope rule are untouched; package-lock.json remains an unknown changed input, so conservative selection is the full executable browser suite."} -->
...
<!-- doc-review {"version":1,"fingerprint":"c1b98d8d8defb22fdd22ea36a6dcc2995732bbcbcec47d956c3f47692def200a","dependencies":"docs/development/.reviews/README/shared-verification-window.json","dependencyDigest":"777a559d5c16cf03b4ef81d8dcb00804b1fcf6a78e18afac49c47b4bdbe61e1f","disposition":"still accurate","rationale":"Unchanged prose. Only package.json's version field moved, 0.4.0 to 0.5.0; no file under scripts/verification-window.mjs, scripts/when-quiet.mjs or scripts/verify-host.mjs changed. The lease, the owner PID and declared-intent notice, the per-tier wait maxima, the nested-command inheritance, the abandonment and recovery semantics, the verify:host readiness probe and the --when-quiet pre-capture poll this section documents are all independent of the package version."} -->
...
<!-- doc-review {"version":1,"fingerprint":"cd02b00fff301faf78935ede9d604f5a892c6a10299141633666ef95e3a2a122","dependencies":"docs/development/.reviews/README/isolated-candidate-completion.json","dependencyDigest":"58c90e137087313d26cb5453eed6ba185f2bd9f2baadb5aa07287347c245f3f0","disposition":"still accurate","rationale":"Unchanged prose. The changed dependencies are scripts/manifest.json, whose whole diff is 15 consumerSourceHash values, and the regenerated src/model/generated/blueprint-validator.mjs, whose only difference from the previous file is the schema $comment now reading save version 4 instead of 3 inside its embedded literal; patching the old file with that one substitution reproduces the new one byte for byte, so no validator branch, accepted shape or rejection moved. Nothing about worktrees, candidate capture, candidate keys, the .metadata_never_index marker, priority files, receipts, receipt reuse, attestation, stacking or landing changed: no file under scripts/candidate*.mjs, scripts/verification-*.mjs, scripts/verify-*.mjs or scripts/land.mjs moved in this closure. Both movers are ordinary selection and candidate-identity inputs this section already describes, and this candidate's evidence will apply only to its own recorded bytes."} -->
