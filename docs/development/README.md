...
## Milestone status
<!-- doc-review {"version":1,"fingerprint":"8dc261f8c59984e12f69db377b831aed8e7033941625511e424b0824f9fda5f0","dependencies":"docs/development/.reviews/README/milestone-status.json","dependencyDigest":"95989243d02985e9a0fb16e17724dcc04c56119c0a58dfaac72739226615d7db","disposition":"still accurate","rationale":"Hosted-ready check waits (claude/hosted-check-waits on 458e9b3c, stacked on q44-release-reuse): scripts/manifest.json changed only in `hostProfiles.github-ubuntu-2cpu` (`waitScale`, `liveSliceMs`, `measurementRuns` = the first full-route nightly 34921754847) and in regenerated scope digests; milestone allocation, bars and gates are untouched."} -->
...
<!-- doc-review {"version":1,"fingerprint":"b5596ee067c2f08263026c3d6076039afc863e298a2a6635b40c90afa1630e56","dependencies":"docs/development/.reviews/README/verify-a-change.json","dependencyDigest":"ee901d41bdc33985df456ce1cf1cdfb46a57dc479f98da992b03b735c375590d","disposition":"still accurate","rationale":"Hosted-ready check waits (claude/hosted-check-waits on 458e9b3c, stacked on q44-release-reuse): the shared browser session now scales every page deadline by the profile's registered `waitScale` (clamped to 60 % of the row budget), budgets workshop startup separately from the interaction deadline, records `waitScale`/`liveWaits`/`targetGeometry` in its evidence, and refuses a wait scale that did not come from the suite; `liveWait` takes its slice from `liveSliceMs` and samples frame gaps; `host-profile.mjs` registers the two facts and passes them to check children as numbers. None of the commands this section names, their order or what each tier verifies changed; `npm run ci` gained no flag here (that is decision 45's follow-up)."} -->
...
<!-- doc-review {"version":1,"fingerprint":"db98f43227b2097ca66a72c86ab803eca0ca1ce1c14c0bdbe643c8b449aa39ae","dependencies":"docs/development/.reviews/README/browser-execution-and-scope.json","dependencyDigest":"ef9359124ee887089a58a270ed9eb8f5b5b1e4eca450eedd5e8a8c0f5f216b10","disposition":"updated","rationale":"Hosted-ready check waits (claude/hosted-check-waits on 458e9b3c, stacked on q44-release-reuse): the hosted-profile paragraph now states the wait scale, the startup budget, the row-budget bound, the unrouted-literal rule, the live slice, the two child variables and their refusals, and the `targetGeometry` verdict on an expired action; the earlier sentence that in-script deadlines are not scaled is retired. Scheduling, selection and witness policy prose unchanged; scope rows regenerated (19 hash-only rows, no consumer-set change)."} -->
...
<!-- doc-review {"version":1,"fingerprint":"152e02cd9fc06c0541091f520e8c59401bc77c619415b8836c83d94477024421","dependencies":"docs/development/.reviews/README/isolated-candidate-completion.json","dependencyDigest":"f363d4b1710eacd58adb76a438cf233dcb6c8b3fe1d8dc9445c4d62281ef9e5a","disposition":"still accurate","rationale":"Hosted-ready check waits (claude/hosted-check-waits on 458e9b3c, stacked on q44-release-reuse): the shared browser session now scales every page deadline by the profile's registered `waitScale` (clamped to 60 % of the row budget), budgets workshop startup separately from the interaction deadline, records `waitScale`/`liveWaits`/`targetGeometry` in its evidence, and refuses a wait scale that did not come from the suite; `liveWait` takes its slice from `liveSliceMs` and samples frame gaps; `host-profile.mjs` registers the two facts and passes them to check children as numbers. Candidate capture, source binding, drift rejection and the tier commands are unchanged; completion tiers additionally refuse `SIMULACRUM_BROWSER_WAIT_SCALE`/`LIVE_SLICE_MS`/`ROW_BUDGET_MS` exactly as they refuse the profile, which this section already describes as the profile rule; assembly-ux-cases only routes one literal through `waitBudget`; verification-environment gained an exemption for the colour variables the format gate pins."} -->
