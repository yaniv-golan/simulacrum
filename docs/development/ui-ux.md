...
<!-- doc-review {"version":1,"fingerprint":"c943efea16d55c2097caffd5288346e083377000518454b7229daeeff0ddf8e4","dependencies":"docs/development/.reviews/ui-ux/current-surfaces-and-lifecycle.json","dependencyDigest":"81f872783f56ad154a45d1e6b5dbdeb49c338e028e8bb740d737c61c5897574d","disposition":"updated","rationale":"Updated: this section's placing-row paragraph now names the band the row reserves as the Sound band it shares the bench's bottom edge with, so a longer state sentence wraps inside the free bench instead of covering Sound, and records that the coordinates card goes away with the fields it edits once the part is placed; it also narrows the vocabulary claim to what the code does, with both homes reading their state word and their confirming control's name from the shared placement vocabulary, the surface panel its instruction sentence too, while the row's role=status region carries the state word alone. Alongside it the section now describes Learn & examples as a bounded picker whose rows scroll beneath a fixed header, and keeps the per-tile favourite star. Its changed dependencies implement exactly that: workshop.css (the reserved inset and the picker rules), workshop-view.mjs and workbench-content.mjs (the picker and its close listener), scripts/catalog-browser-actions.mjs (helpers that open a row) and the linked playtesting.md#release-operations. The receipt is re-recorded on this tip because the merge left the marker and sidecar matching neither parent."} -->
