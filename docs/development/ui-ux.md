...
<!-- doc-review {"version":1,"fingerprint":"d04b8268438944172bf557875002209427a40e94a1b7a9cd27799ecfe311ee55","dependencies":"docs/development/.reviews/ui-ux/learning-content-policy.json","dependencyDigest":"d92d73a9dd78ca3e4dcb8965b1e9a2246402be51e2dca80c9f2919feec5aaf40","disposition":"still accurate","rationale":"Unchanged prose, and the recorded decision for this candidate remains no entry: the elastic cord adds, extends and replaces no teaching, example, experiment, challenge or contextual invitation. The one changed dependency is src/presentation/workshop-view.mjs, where this candidate excludes cord edges from the wiring overlay and the merged integration renders the existing Learn entries as rows that collapse on close and derives a parameter field's range from its own menu. The merged integration's gear tooth-size mismatch stays an error message rather than teaching, so the admission rules and the record they require are unaffected."} -->
...
<!-- doc-review {"version":1,"fingerprint":"271ace7214746c40fd125a1b111f7f33205c94a7b9f732c5a56e7409c968e49c","dependencies":"docs/development/.reviews/ui-ux/current-surfaces-and-lifecycle.json","dependencyDigest":"238301811708527eff0d50d24a684faee4582255177998717b0720736533ccb0","disposition":"updated","rationale":"Re-recorded because the merge left this section's marker and its dependency sidecar matching neither parent, so the previous receipt was malformed rather than merely stale. Updated, and the section now carries both sides of the stack. From the merged parametric-gear integration: one catalog card covers every ratio because tooth count and tooth size are authored settings, so no numeric query names a part type; tooth size is a two-value menu and a parameter declaring a menu gives its control that menu's bounds and spacing, so an off-menu number is reported by the field and never sent as an edit; and mismatched tooth size and an out-of-tolerance centre distance are reported on the mesh row in its own words, driving nothing and never refusing the setting that caused them. From this candidate: no player-facing surface, region or visibility lifecycle changed, because the elastic cord still has no palette card, panel or control. Two part-scoped surfaces gain correct output rather than new copy owners -- the inspector summary line and the exploded view's Trace connections list both name it Elastic cord through the shared connection labels instead of printing a raw kind, the trace row naming its mounting region because endpointName resolves a surface endpoint -- while the per-port Trace and Disconnect actions are not reachable for a cord at all, since they are built from portConnections and a cord is surface-bound and occupies no port. The wiring overlay no longer draws a cord as a wire. The movers behind those statements are src/presentation/workshop-view.mjs and scripts/gear-browser-cases.mjs, with the gear model set and the regenerated schema artefacts underneath."} -->
