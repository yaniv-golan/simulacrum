...
<!-- doc-review {"version":1,"fingerprint":"363a863f9a8c968eed6fbeaf5ddc9c6f4f3b726bc289ec3e0d05e1f0102b9a70","dependencies":"docs/development/.reviews/ui-ux/current-surfaces-and-lifecycle.json","dependencyDigest":"dd74697a9cf3003fe3e6ff976a368f4aca93929a517a36f7b06c17318f0b067c","disposition":"updated","rationale":"Updated, and re-recorded because the merge left the marker and its dependency sidecar matching neither parent. The section now carries both sides of the stack. From the merged placement-strip: the placing row reserves the Sound band it shares the bench's bottom edge with so a longer state sentence wraps inside the free bench, the coordinates card opens above the row and leaves with the fields it edits once the part is placed, both homes of the act read their state word and their confirming control's name from the shared placement vocabulary while the row's role=status region carries the state word alone, Learn & examples is a bounded picker whose rows scroll beneath a fixed header, and the per-tile favourite star stays. Added on this tip: while a placement is live the ghost casts the key light's shadow and drops a dashed line from its lowest point to a landing ring directly beneath it, and a transient chip at the aimed point names the part the mounting pad will land on and the gap left, to a tenth of a millimetre so a fraction of a millimetre cannot read as contact; the chip never takes the pointer and leaves with the placement, and the drop-line is drawn whether or not the quality level allows shadows. The cues are a transient overlay owned by the placement itself and displace no existing surface."} -->
