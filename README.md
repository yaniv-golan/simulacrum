...
- A multi-selection exposes exact XYZ pivot coordinates, alignment to the mint primary component, and equal-spacing distribution along X, Y, or Z. Each operation is one undoable history entry.
...
- The workbench is a finite raised platform, not an infinite ground plane. Drive beyond its edge to test the drop and landing; the mission display switches from `ROVER FALLING` to `FIELD TERRAIN` when the rover reaches the grass.
...
- Press `F` to frame the selected component. When nothing is selected, `F` frames the complete machine. `Shift+F` follows the selected component, and double-clicking a component selects and focuses it.
...
- Keyboard shortcuts: `O` and `P` activate Orbit/Pan; `X` toggles Exploded View; arrow keys orbit; `WASD` moves across the ground plane; `Q`/`E` move down/up; `+`/`−` zoom; `Home` resets the camera.
