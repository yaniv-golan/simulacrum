...
| bar | command | asserts |
|---|---|---|
| **L0** | `npm run bar:L0` | **The Course, complete circuit, driven by the WHEELED ROVER.** Due M5, with its evaluator, frozen dimensions and power margin due M5 too. An **apparatus smoke test**: it shows the course is self-consistent and achievable by *something*. It does **not** exercise swing clearance, intermittent support or articulated load transfer, so a later walker failure still admits engine causes — keep competing hypotheses. And a rover failure may be the rover's controller, gearing or power, not the course. |
