...
import { compareCanonicalIds } from "../model/canonical-id-contract.js";
...
export class ControllerRuntimeManager {
  ...
  exportState() {
    return issueInertPlainData(
      [...this.runtimes.values()]
        .sort((left, right) =>
          compareCanonicalIds(left.controllerId, right.controllerId),
        )
        .map((runtime) => ({
          controllerId: runtime.controllerId,
          bindingManifestIdentity: runtime.bindingManifestIdentity,
  ...
  validateState(state) {
    const detachedState = requireInertPlainData(state, {
      code: "INVALID_CONTROLLER_CHECKPOINT_PLAIN_DATA",
      message:
        "Controller checkpoint must be serialized JSON or an exported immutable state",
    });
    if (
      !Array.isArray(detachedState) ||
      detachedState.length !== this.runtimes.size
    )
      throw new Error("controller checkpoint does not match attached programs");
    const byId = new Map();
    for (const record of detachedState) {
      ...
      byId.set(record.controllerId, {
        ...
        commands,
        // Preserve the issued nested root; structuredClone intentionally
        // strips the unforgeable inert-boundary provenance.
