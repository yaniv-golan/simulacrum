...
function record(value,keys) {
 if(!value||typeof value!=='object'||Array.isArray(value)||Object.keys(value).length!==keys.length||keys.some(key=>!Object.hasOwn(value,key)))throw new TypeError('invalid physics fields');
}
function vector(value,size=3){if(!Array.isArray(value)||value.length!==size||value.some(number=>!Number.isFinite(number)))throw new TypeError('invalid numeric vector');return [...value];}
function quaternion(value){const result=vector(value,4);if(Math.abs(Math.hypot(...result)-1)>1e-6)throw new TypeError('invalid unit quaternion');return result;}
function xyzw(value){return {x:value[0],y:value[1],z:value[2],w:value[3]};}
function rotationArray(value){return [value.x,value.y,value.z,value.w];}
function unit(value){const v=vector(value),length=Math.hypot(...v);if(!Number.isFinite(length)||length<1e-12)throw new TypeError('invalid axis');return v.map(n=>n/length);}
...
function rotate(q,v){return multiply(multiply(q,[...v,0]),conjugate(q)).slice(0,3);}
function xyz(value){return {x:value[0],y:value[1],z:value[2]};}
function array(value){return [value.x,value.y,value.z];}
function hash(bytes){let h=2166136261;for(const byte of bytes)h=Math.imul(h^byte,16777619);return h>>>0;}
function encode(payload,handles,configuration){
 ...
 const bytes=new Uint8Array(12+metadata.length+payload.length),view=new DataView(bytes.buffer);
 view.setUint32(0,0x53494d31);view.setUint32(4,metadata.length);bytes.set(metadata,12);bytes.set(payload,12+metadata.length);view.setUint32(8,hash(bytes.subarray(12)));return bytes;
}
function decode(input){
 if(!(input instanceof Uint8Array)||input.length<13||input.length>32*1024*1024)throw new TypeError('invalid physics snapshot');
 const bytes=new Uint8Array(input),view=new DataView(bytes.buffer),size=view.getUint32(4);
 if(view.getUint32(0)!==0x53494d31||size===0||12+size>=bytes.length||view.getUint32(8)!==hash(bytes.subarray(12)))throw new TypeError('invalid physics snapshot envelope');
 const metadata=JSON.parse(new TextDecoder('utf-8',{fatal:true}).decode(bytes.subarray(12,12+size)));
 record(metadata,['version','handles','configuration']);
 if(metadata.version!==1||!Array.isArray(metadata.handles)||metadata.handles.length>MAX_BODIES||metadata.handles.some(handle=>!Number.isFinite(handle))||new Set(metadata.handles).size!==metadata.handles.length)throw new TypeError('invalid physics snapshot handles');
 return {handles:metadata.handles,configuration:metadata.configuration,payload:bytes.slice(12+size)};
}
export async function createPhysicsWorld(configuration){
 record(configuration,['gravity','bodies','joints']);const gravity=vector(configuration.gravity);
 if(!Array.isArray(configuration.bodies)||configuration.bodies.length>MAX_BODIES)throw new TypeError('invalid bodies');
 const descriptions=configuration.bodies.map(body=>{
  record(body,['shape','position','rotation','velocity','mass','halfExtents','fixed','friction','restitution']);
  const rotation=quaternion(body.rotation),position=vector(body.position),velocity=vector(body.velocity),halfExtents=vector(body.halfExtents);
  if(!Number.isFinite(body.mass)||body.mass<=0||halfExtents.some(value=>value<=0)||typeof body.fixed!=='boolean'||!Number.isFinite(body.friction)||body.friction<0||!Number.isFinite(body.restitution)||body.restitution<0||body.restitution>1)throw new TypeError('invalid body properties');
  if(!['box','cylinder'].includes(body.shape)||(body.shape==='cylinder'&&halfExtents[1]!==halfExtents[2]))throw new TypeError('invalid physical shape');
  return {shape:body.shape,position,rotation,velocity,halfExtents,mass:body.mass,fixed:body.fixed,friction:body.friction,restitution:body.restitution};
 });
 if(!Array.isArray(configuration.joints)||configuration.joints.length>8192)throw new TypeError('invalid joints');
 const joints=configuration.joints.map(joint=>{
  if(!['fixed','revolute'].includes(joint?.kind))throw new TypeError('invalid joint kind');
  record(joint,joint.kind==='fixed'?['kind','a','b','anchorA','anchorB','rotationA','rotationB']:['kind','a','b','anchorA','anchorB','axisA','axisB']);
  if(!Number.isInteger(joint.a)||!Number.isInteger(joint.b)||joint.a<0||joint.b<0||joint.a>=descriptions.length||joint.b>=descriptions.length||joint.a===joint.b)throw new TypeError('invalid joint body binding');
  const common={kind:joint.kind,a:joint.a,b:joint.b,anchorA:vector(joint.anchorA),anchorB:vector(joint.anchorB)};
  return joint.kind==='fixed'?{...common,rotationA:quaternion(joint.rotationA),rotationB:quaternion(joint.rotationB)}:{...common,axisA:unit(joint.axisA),axisB:unit(joint.axisB)};
 });
 ...
 try{
  ...
  for(const body of descriptions){
   ...
   let collider;
   if(body.shape==='box')collider=RAPIER.ColliderDesc.cuboid(...body.halfExtents).setMass(body.mass);
   else {
    ...
    const [h,r]=body.halfExtents,segments=CYLINDER_SEGMENTS;
    const vertices=new Float32Array(Array.from({length:segments*2},(_,i)=>{const angle=(i%segments)*2*Math.PI/segments;return [i<segments?-h:h,r*Math.cos(angle),r*Math.sin(angle)];}).flat());
    const transverse=body.mass*(3*r*r+4*h*h)/12;
    collider=RAPIER.ColliderDesc.convexHull(vertices).setMassProperties(body.mass,{x:0,y:0,z:0},{x:body.mass*r*r/2,y:transverse,z:transverse},{x:0,y:0,z:0,w:1});
   }
   world.createCollider(collider.setFriction(body.friction).setRestitution(body.restitution),rigidBody);handles.push(rigidBody.handle);
  }
  for(const joint of joints){
   const data=joint.kind==='fixed'?RAPIER.JointData.fixed(xyz(joint.anchorA),xyzw(joint.rotationA),xyz(joint.anchorB),xyzw(joint.rotationB)):RAPIER.JointData.revoluteWithAxes(xyz(joint.anchorA),xyz(joint.anchorB),xyz(joint.axisA),xyz(joint.axisB));
   const connection=world.createImpulseJoint(data,world.getRigidBody(handles[joint.a]),world.getRigidBody(handles[joint.b]),true);connection.setContactsEnabled(false);jointHandles.push(connection.handle);
  }
 ...
 function dimensions(collider){
  ...
  if(collider.shapeType()===RAPIER.ShapeType.ConvexPolyhedron)return {vertices:Array.from(collider.vertices()),indices:Array.from(collider.indices())};
 ...
 function alive(){if(!world)throw new Error('physics world disposed');}
 function readWorld(candidate,mapping){return mapping.map(handle=>{
  const body=candidate.getRigidBody(handle);if(!body)throw new Error('snapshot body missing');
  const rotation=body.rotation();return {position:array(body.translation()),rotation:[rotation.x,rotation.y,rotation.z,rotation.w],velocity:array(body.linvel()),angularVelocity:array(body.angvel()),mass:body.mass()};
 });}
 ...
 return Object.freeze({
  ...
  jointState(index){
   ...
   const config=joints[index],joint=world.getImpulseJoint(jointHandles[index]),a=bodyAt(config.a),b=bodyAt(config.b);
   const qa=rotationArray(a.rotation()),qb=rotationArray(b.rotation()),axis=unit(rotate(qa,config.axisA));
   const local=multiply(conjugate(multiply(qa,rotationArray(joint.frameX1()))),multiply(qb,rotationArray(joint.frameX2())));
   const raw=2*Math.atan2(local[0],local[3]),angle=Math.atan2(Math.sin(raw),Math.cos(raw));
   const wa=array(a.angvel()),wb=array(b.angvel()),speed=axis.reduce((sum,value,i)=>sum+value*(wb[i]-wa[i]),0);
   return {angle,speed,effectiveInverseInertia:getAxisInverseInertia(config.a,axis)+getAxisInverseInertia(config.b,axis)};
  },
  ...
  applyImpulse(index,impulse){alive();if(!Number.isInteger(index)||index<0||index>=handles.length)throw new TypeError('invalid body index');const value=vector(impulse);if(value.some(number=>!Number.isFinite(Math.fround(number))))throw new RangeError('impulse exceeds physics numeric range');world.getRigidBody(handles[index]).applyImpulse(xyz(value),true);},
  dispose(){if(world){world.free();world=null;handles=[];}},
 });
}
