    ...
    run(snapshot) {
      ...
      for (const reading of previous.readings) {
        if (
          ...
          fail('INVALID_CONTROLLER_SNAPSHOT');
        readings.set(reading.node, reading.speed);
      }
      const views = compiled.map((program) => {
        ...
        return data(
          {
            tick: previous.tick,
            inputs: program.inputs.map((input) => ({ node: input, speed: readings.get(input) })),
          },
      ...
      for (const [index, program] of compiled.entries()) {
        ...
        for (const command of output) {
          ...
          result.push({ node: command.node, duty: command.duty });
        }
      }
