...
import { surfaceRegions, resolveSurfaceEndpoint, placementEnvelopes, solidsOverlap } from '../model/surfaces.mjs';
...
export function createWorkshopView(root,{onCommand,onSave,onLoad,onFailure,onRecording,onInteraction,getCursor,guideSteps=[]}) {
 ...
 filebar.append(newButton,button('Save',async()=>{try{await onSave();}catch{setMessage('The machine could not be saved. Try again.');}}),button('Load',()=>loadInput.click()),failureButton,loadInput);
 ...
 if(step){guide.append(element('p','',step.description));const next=button(step.label,async()=>{editing.clearPreview();const result=await send({type:'guide-step'});if(result?.ok){const command=step.commands.find(c=>c.type==='connect'),edge=command&&frame.metadata.blueprint.connections.find(c=>c.id===command.id);if(edge){select(edge.b.part);sourcePort={...edge.b};inspectorKey='';refreshInspector();guideReceipt={...edge,message:guideConnectionMessage(edge)};guidePulseStarted=performance.now();showGuideConnection(guideReceipt,true);}refreshGuide();editing.focus();}},'primary');next.dataset.command='guide-step';next.disabled=frame?.metadata.mode!=='build';const preview=()=>{if(step.part)editing.showPreview([step.part]);const connection=step.commands.find(c=>c.type==='connect');if(connection)showGuideConnection(connection,false);};const leave=()=>{if(step.part)return;editing.clearPreview();showGuideConnection(guideReceipt,true);};next.addEventListener('pointerenter',()=>{if(step.part)preview();});next.addEventListener('pointermove',event=>{if(!step.part&&(event.movementX||event.movementY))preview();});next.addEventListener('focus',preview);next.addEventListener('pointerleave',leave);next.addEventListener('blur',leave);guide.insertBefore(next,guide.children[1]);if(step.part)preview();}else{guide.append(element('h2','','Ready for a rolling test'),element('p','','Press Run. This three-wheel machine travels in a curve. Pause and return to Build to try a change. The cell, mounts and wheels all remain editable.'));}
 guide.append(button('Leave guide',()=>{guideActive=false;empty.hidden=frame.metadata.blueprint.parts.length>0;guideReceipt=null;showGuideConnection(null);editing.clearPreview();refreshGuide();},'quiet'));}
 ...
 const inspectionFill=new THREE.DirectionalLight(0xc5e4ef,1.2);scene.add(inspectionFill,inspectionFill.target);
 ...
 const partResources=createResourceCache({key:partAppearanceKey,create:part=>{const mesh=createPartMesh(part),display=new THREE.Group();display.add(mesh);scene.add(display);return mesh;},dispose:mesh=>{scene.remove(mesh.parent);disposePart(mesh);}});
 const meshes=partResources.values,wires=new THREE.Group();scene.add(wires);const connectionView=createConnectionView(wires);const portCues=new THREE.Group();scene.add(portCues);let portCueKey='',previewEndpoint=null,socketPreview=null;
 editing=createEditingControls({scene,camera,renderer,orbit:controls,getPart:id=>frame?.metadata.blueprint.parts.find(p=>p.id===id),getBlueprint:()=>frame.metadata.blueprint,getMode:()=>exploded?'inspection':frame?.metadata.mode,getMeshes:()=>meshes,getViewportInsets:()=>{const rect=stage.getBoundingClientRect();let top=100,bottom=70;for(const node of document.querySelectorAll('.edit-toolbar,.inspection-banner,.playtest-panel')){if(node.hidden||!node.getClientRects().length)continue;const box=node.getBoundingClientRect();if(box.top<rect.top+rect.height/2)top=Math.max(top,box.bottom-rect.top+16);else bottom=Math.max(bottom,rect.bottom-box.top+16);}return {top,bottom,left:24,right:24};},onCommit:send});
 ...
 renderer.domElement.addEventListener('pointerup',event=>{if(surfacePointer===null||event.button!==0)return;event.stopImmediatePropagation();surfacePointer=null;surface.endPointer(event);controls.enabled=true;if(renderer.domElement.hasPointerCapture(event.pointerId))renderer.domElement.releasePointerCapture(event.pointerId);},true);
 ...
 function releaseSurfacePointer(){const id=surfacePointer;surfacePointer=null;if(id!==null&&renderer.domElement.hasPointerCapture(id))renderer.domElement.releasePointerCapture(id);controls.enabled=true;}
 function cancelInteraction(){endDirectDrag(false);releaseSurfacePointer();surface?.cancel(false);draggingType=null;placementCue.hidden=true;editing?.cancel();controls.enabled=true;}
 renderer.domElement.addEventListener('pointercancel',cancelInteraction);
 renderer.domElement.addEventListener('lostpointercapture',()=>{if(surfacePointer!==null||directDrag)cancelInteraction();});
 ...
 function refreshInspector(){
  ...
  const rename=button('Rename',()=>{
   const editor=element('form','part-name-editor'),input=element('input');input.value=part.name;input.maxLength=128;input.required=true;input.setAttribute('aria-label','Part name');
   const save=element('button','','Save name');save.type='submit';const cancel=button('Cancel',()=>refreshName());
   function refreshName(){inspectorKey='';refreshInspector();}
   input.addEventListener('keydown',event=>{if(event.key==='Escape'){event.preventDefault();event.stopPropagation();refreshName();}});
   editor.addEventListener('submit',async event=>{event.preventDefault();if(!input.value.trim()){input.setCustomValidity('Enter a part name.');input.reportValidity();return;}const result=await send({type:'rename',id:part.id,name:input.value});if(result?.ok)refreshName();});
   input.addEventListener('input',()=>input.setCustomValidity(''));editor.append(input,save,cancel);rename.replaceWith(editor);input.focus();input.select();
  },'quiet');rename.setAttribute('aria-label','Rename part');rename.disabled=!editable;identity.append(rename);
  ...
  if(surfaceRegions(part).length){const mounting=element('section','mount-status');mounting.setAttribute('aria-label','Mounting');const edges=frame.metadata.blueprint.connections.filter(c=>c.kind==='fixed'&&(c.a.part===part.id||c.b.part===part.id));
   const axleEdges=frame.metadata.blueprint.connections.filter(c=>c.kind==='shaft'&&(c.a.part===part.id||c.b.part===part.id));if(!edges.length&&!axleEdges.length)mounting.append(element('p','','Unattached'));for(const edge of edges.length?[]:axleEdges){const peer=parts.find(p=>p.id===(edge.a.part===part.id?edge.b.part:edge.a.part));mounting.append(element('p','',`Axle attached to ${peer.name}`),element('p','parameter-help','The axle holds these parts together and allows rotation.'));}
   for(const edge of edges){const own=edge.a.part===part.id?edge.a:edge.b,other=edge.a.part===part.id?edge.b:edge.a,peer=parts.find(p=>p.id===other.part),row=element('div','mount-relationship');row.append(element('span','',`Bolted to ${peer.name} · ${endpointName(peer,other)}`));if(editable&&edge.b.part===part.id&&own.surface&&other.surface)row.append(button('Adjust mount',()=>beginSurface(part.id,{replaceConnection:edge.id})));if(editable)row.append(button('Detach',()=>send({type:'disconnect',id:edge.id}),'quiet'));mounting.append(row);}
   if(editable&&!edges.length&&!axleEdges.length){const snap=button('Snap to surface',()=>beginSurface(part.id));snap.dataset.command='snap-surface';mounting.append(snap);}
  ...
  for(const port of orderedPorts){
   ...
   if(!occupied(part,port)){
    ...
    let count=0;for(const target of parts)if(target.id!==part.id)for(const targetPort of CATALOG[target.type].ports)if(compatible(port,targetPort)&&!occupied(target,targetPort)&&!connections.some(connection=>[connection.a,connection.b].some(endpoint=>endpoint.part===target.id&&endpoint.port===targetPort.id))){
     ...
     count++;const targetButton=button(`${mechanical?'Attach to':'Wire'} ${target.name} · ${targetPort.id}${mechanical?'':' (parts stay put)'}`,async()=>{const result=await send({type:'connect',...endpoints()});if(result?.ok===true){socketPreview=null;editing.clearPreview();sourcePort=null;previewEndpoint=null;snapNotice.hidden=true;inspectorKey='';refreshInspector();setMessage(mechanical?`${movement} moved to ${stationary.name}. Connected. Undo to restore.`:`${part.name} wired to ${target.name}. Both parts stayed in place.`);}},'target-button');
     if(mechanical){const detail=element('span','snap-movement',`Moves ${movement}`);targetButton.append(detail);}
     const preview=()=>{previewEndpoint={part:target.id,port:targetPort.id};if(mechanical){let moved=[];try{const {a,b}=endpoints(),next=snapConnection(frame.metadata.blueprint,a,b);moved=next.parts.filter((p,i)=>JSON.stringify(p)!==JSON.stringify(frame.metadata.blueprint.parts[i]));const candidate=structuredClone(next);candidate.connections.push({id:'preview-connection',kind:port.kind,a,b});compileAssembly(candidate);editing.showPreview(moved);socketPreview={button:targetButton,valid:true};snapNotice.textContent=`Connection preview · Moves ${movement}. ${stationary.name} stays. Click the preview or Attach to confirm; Esc cancels.`;snapNotice.hidden=false;}catch(error){editing.showPreview(moved,{color:0xff836f});socketPreview={button:targetButton,valid:false};const movingIds=new Set(moved.map(p=>p.id)),obstacle=frame.metadata.blueprint.parts.find(p=>!movingIds.has(p.id)&&moved.some(m=>placementEnvelopes(m).some(a=>placementEnvelopes(p).some(b=>solidsOverlap(a,b)))));if(obstacle)editing.showPreview([...moved,obstacle],{color:0xff836f});snapNotice.textContent=obstacle?`Cannot attach: ${movement} overlaps ${obstacle.name}. Adjust its mount or move it clear first.`:`Cannot attach: ${explainFailure(error,frame.metadata.blueprint)}`;snapNotice.hidden=false;}}targetButton.classList.add('previewing');};targetButton.addEventListener('pointerenter',preview);targetButton.addEventListener('focus',preview);targetButton.dataset.targetPartId=target.id;targetButton.dataset.targetPortId=targetPort.id;targets.append(targetButton);
    }
   ...
   explanation.append(button('Cancel connection',()=>{socketPreview=null;snapNotice.hidden=true;editing.clearPreview();previewEndpoint=null;sourcePort=null;inspectorKey='';refreshInspector();right.querySelector(`[data-port-id="${port.id}"]`)?.focus({preventScroll:true});},'quiet'));
  }
 ...
 function rebuildMeshes(blueprint){partResources.reconcile(blueprint.parts);refreshPartList();}
 ...
 function updateConnections(){
  ...
  for(const connection of frame.metadata.blueprint.connections){
   ...
   specs.push({id:connection.id,kind:connection.kind,ends,failed:diagnostic?.reasonCode!=='OK',exploded:explodeAmount>0,highlighted:connection.id===tracedConnection||(!tracedConnection&&[connection.a.part,connection.b.part].includes(selected))});
  }
 ...
 function setExploded(on,immediate=false){surface?.cancel(false);
  ...
  explodeFrom=explodeAmount;explodeStarted=performance.now();exploded=on;inspectionBanner.hidden=!on;const cameraFrom={position:camera.position.clone(),target:controls.target.clone()};
  if(on){const previous=explodeAmount;explodeAmount=1;applyExploded();editing.focus({recover:false});explodeCameraTween={from:cameraFrom,to:{position:camera.position.clone(),target:controls.target.clone()},started:performance.now()};explodeAmount=previous;applyExploded();camera.position.copy(cameraFrom.position);controls.target.copy(cameraFrom.target);controls.update();}
 ...
 function render(next){const previousCount=frame?.metadata.blueprint.parts.length??0;const previousMode=frame?.metadata.mode;frame=next;surface.refresh();const blueprint=frame.metadata.blueprint,key=JSON.stringify(blueprint);if((exploded||explodeAmount)&&(key!==blueprintKey||frame.metadata.mode==='run'))setExploded(false,true);if(key!==blueprintKey){if(guideVisual)showGuideConnection(null);blueprintKey=key;const added=blueprint.parts.filter(part=>!meshes.has(part.id)).at(-1);if(added)selected=added.id;else if(!blueprint.parts.some(part=>part.id===selected))selected=null;if(sourcePort&&!blueprint.parts.some(part=>part.id===sourcePort.part))sourcePort=null;rebuildMeshes(blueprint);inspectorKey='';}
  ...
  if(blueprint.parts.length>previousCount)editing.focus();explodeButton.disabled=frame.metadata.mode==='run'||blueprint.parts.length<2;refreshSelectionVisuals();if(previousMode&&previousMode!=='build'&&frame.metadata.mode==='build')editing.focus();editing.select(selected);undo.disabled=frame.metadata.mode!=='build'||!frame.metadata.editing?.undoCount;redo.disabled=frame.metadata.mode!=='build'||!frame.metadata.editing?.redoCount;refreshGuide();updateConnections();refreshInspector();refreshLive();refreshHealth();failureButton.hidden=frame.status!=='failed';empty.hidden=blueprint.parts.length>0||guideActive;tickLabel.textContent=`Tick ${frame.tick}`;modeLabel.textContent=frame.status==='failed'?'STOPPED':frame.metadata.mode.toUpperCase();run.disabled=frame.metadata.mode==='run';pause.disabled=frame.metadata.mode!=='run';stepButton.disabled=frame.metadata.mode!=='paused';build.classList.toggle('active',frame.metadata.mode==='build');for(const b of palette.querySelectorAll('button'))b.disabled=frame.metadata.mode!=='build';for(const b of more.querySelectorAll('button'))b.disabled=frame.metadata.mode!=='build';
 }
