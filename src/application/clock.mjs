...
export function createClock(session, {requestFrame, cancelFrame, render}) {
  let handle=null,previous=null,running=false;
  function frame(now) {
    if(!running)return;
  ...
  return Object.freeze({
    start(){if(running)return;running=true;previous=null;handle=requestFrame(frame);},
    pause(){running=false;if(handle!==null)cancelFrame(handle);handle=null;previous=null;},
    advanceTime(ms){session.advanceTime(ms);render();},
    running:()=>running,
  });
}
