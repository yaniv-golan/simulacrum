...
function jsonCopy(value, ancestors = new Set()) {
  ...
  if (Reflect.ownKeys(value).some((key) => typeof key !== 'string'))
    throw new Error('JSON cannot contain symbol keys');
  if (
    Array.isArray(value) &&
  ...
  if (
    ...
    throw new Error('JSON cannot contain hidden properties');
  for (const key of Object.keys(value)) {
    const descriptor = Object.getOwnPropertyDescriptor(value, key);
    if (!('value' in descriptor)) throw new Error('JSON cannot contain accessors');
    Object.defineProperty(result, key, {
      value: jsonCopy(descriptor.value, ancestors),
      enumerable: true,
      configurable: true,
      writable: true,
    });
...
export function createInteractionRecorder({
  ...
  now = () => Date.now(),
  wallNow = () => Date.now(),
  idFactory = () => `interaction-${Date.now()}`,
  maxEvents = 10000,
  maxBytes = 2000000,
}) {
  if (
    ...
    throw new Error('Capture limits must be positive integers');
  let capture = null;
  let origin = 0;
  let reason = null;
  let error = null;
  ...
  const fits = (value) =>
    size({
      ...value,
      endedAt: value.endedAt ?? value.startedAt,
      status: 'stopped',
  ...
  const close = (why, finalContext = capture?.finalContext ?? null) => {
    ...
    try {
      ...
      /* The start anchor remains available. */
    }
  ...
  return {
    start(initialContext = null) {
      ...
      try {
        ...
        candidate = {
          version: 1,
          id,
          build: jsonCopy(build),
          startedAt: utc(wallNow()),
          endedAt: null,
          status: 'recording',
          terminationReason: null,
          initialContext: jsonCopy(initialContext),
          finalContext: null,
          events: [],
        };
      } catch (cause) {
      ...
      try {
        origin = now();
        if (!Number.isFinite(origin)) throw new Error('Invalid monotonic clock');
      } catch (cause) {
    ...
    record(kind, data = null, context = null) {
      ...
      try {
        ...
        return fail('invalid-data', cause);
      }
      try {
        const current = now();
        if (!Number.isFinite(current) || !Number.isFinite(current - origin))
          throw new Error('Invalid monotonic clock');
        event.timeMs = Math.max(0, capture.events.at(-1)?.timeMs ?? 0, current - origin);
      } catch (cause) {
        return fail('clock-failure', cause);
      }
    ...
    readLast() {
      try {
        ...
        if (
          ...
          !value.id ||
          !Object.hasOwn(value, 'build') ||
          !Object.hasOwn(value, 'initialContext') ||
          !Object.hasOwn(value, 'finalContext') ||
          !Array.isArray(value.events) ||
          value.events.length > maxEvents ||
          utc(value.startedAt) !== value.startedAt
        )
          return null;
        if (value.status === 'recording') {
          if (value.terminationReason !== null || value.endedAt !== null) return null;
        } else if (
          value.status !== 'stopped' ||
          !reasons.has(value.terminationReason) ||
          utc(value.endedAt) !== value.endedAt
        )
          return null;
        let previous = 0;
