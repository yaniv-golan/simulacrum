...
function commandFailure(error){
 ...
 return result(false,reasonCode,typeof own.path?.value==='string'?own.path.value:'command');
}
...
export async function createWorkshop(input=createEmptyBlueprint('machine','My machine'),identity={}) {
 ...
 const session=await createSession(compiled.configuration,identity,{blueprint:loaded.blueprint,mapping:compiled.mapping,connections:compiled.connections,mode:'build',editing:{undoCount:0,redoCount:0}});
 let busy=false,disposed=false;
 const past=[],future=[],historyLimit=50;
 const retain=(stack,state)=>{stack.push(immutableCopy(state));if(stack.length>historyLimit)stack.shift();};
 ...
 async function act(inputCommand) {
  ...
  let command;
  try{command=immutableCopy(inputCommand);}catch{return result(false,'INVALID_COMMAND','command');}
  if(!command||typeof command!=='object'||Array.isArray(command))return result(false,'INVALID_COMMAND','command');
  busy=true;
  try {
   const current=metadata(),keys=Object.keys(command).sort().join(',');
   if(['run','pause','build'].includes(command.type)) {
    ...
    if(command.type==='build') {
     ...
     await session.replaceConfiguration(next.configuration,{blueprint:current.blueprint,mapping:next.mapping,connections:next.connections,mode:'build',editing:current.editing});
   ...
   if(command.type==='control') {
    ...
    const node=current.blueprint.parts.findIndex(p=>p.id===command.id&&p.type==='commandReceiver');
   ...
   if(['undo','redo'].includes(command.type)) {
    if(keys!=='type')return result(false,'INVALID_COMMAND','command');
    const from=command.type==='undo'?past:future,to=command.type==='undo'?future:past;
    if(!from.length)return result(false,command.type==='undo'?'NOTHING_TO_UNDO':'NOTHING_TO_REDO');
    const blueprint=from.at(-1),candidate=compileAssembly(blueprint);
    await session.replaceConfiguration(candidate.configuration,{blueprint,mapping:candidate.mapping,connections:candidate.connections,mode:'build',editing:{undoCount:command.type==='undo'?past.length-1:Math.min(historyLimit,past.length+1),redoCount:command.type==='redo'?future.length-1:Math.min(historyLimit,future.length+1)}});
    from.pop();retain(to,current.blueprint);return result(true);
   }
   ...
   switch(command.type) {
    ...
    case 'delete':
     ...
     next.connections=next.connections.filter(connection=>connection.a.part!==command.id&&connection.b.part!==command.id);break;
    case 'transform': {
     if(keys!=='id,position,rotation,type')return result(false,'INVALID_COMMAND','command');
     next=transformGroup(current.blueprint,command.id,command.position,command.rotation);break;
    }
    case 'material': {
     if(keys!=='id,material,primitive,type')return result(false,'INVALID_COMMAND','command');
     const part=next.parts.find(part=>part.id===command.id);if(!part)return result(false,'UNKNOWN_PART','id');
     if(typeof command.primitive!=='string')return result(false,'INVALID_COMMAND','primitive');
     Object.defineProperty(part.authoredMaterial,command.primitive,{value:command.material,enumerable:true,writable:true,configurable:true});break;
    }
    case 'surface-mount': {
    ...
    case 'connect': {
     ...
     const port=part&&(command.a?.surface?resolveSurfaceEndpoint(part,command.a):CATALOG[part.type].ports.find(p=>p.id===command.a?.port));
     if(!port)return result(false,'INVALID_ENDPOINT','a');
     if(['fixed','shaft'].includes(port.kind))next=snapConnection(next,command.a,command.b);
     next.connections.push({id:command.id,kind:port.kind,a:command.a,b:command.b});break;
    }
    case 'disconnect':
     if(keys!=='id,type')return result(false,'INVALID_COMMAND','command');
     if(typeof command.id!=='string')return result(false,'INVALID_COMMAND','id');
     if(!next.connections.some(connection=>connection.id===command.id))return result(false,'UNKNOWN_CONNECTION','id');
     next.connections=next.connections.filter(connection=>connection.id!==command.id);break;
    case 'parameter': {
     if(keys!=='id,key,type,value')return result(false,'INVALID_COMMAND','command');
     const part=next.parts.find(p=>p.id===command.id);if(!part)return result(false,'UNKNOWN_PART','id');
     if(typeof command.key!=='string'||!Object.hasOwn(part.parameters,command.key))return result(false,'INVALID_COMMAND','key');
     part.parameters[command.key]=command.value;break;
    }
    case 'load': {
     if(keys!=='save,type')return result(false,'INVALID_COMMAND','command');
     const loaded=loadSave(command.save);if(!loaded.ok)return loaded;next=loaded.blueprint;break;
    }
   ...
   if(command.type==='load'){past.length=0;future.length=0;}
   else if(!sameData(current.blueprint,next)){retain(past,current.blueprint);future.length=0;}
   return result(true);
  }catch(error){return commandFailure(error);}
  finally{busy=false;}
 }
 function available(){if(disposed)reject('SESSION_DISPOSED');if(busy)reject('BUSY');}
 function restore(input){
  available();let checkpoint;
  try{
   checkpoint=immutableCopy(input);
   const data=checkpoint.metadata;
   if(!data||Object.keys(data).sort().join(',')!=='blueprint,connections,editing,mapping,mode'||!['build','run','paused'].includes(data.mode))reject('INVALID_CHECKPOINT','metadata');
   if(!data.editing||Object.keys(data.editing).sort().join(',')!=='redoCount,undoCount'||!Object.values(data.editing).every(value=>Number.isSafeInteger(value)&&value>=0&&value<=historyLimit))reject('INVALID_CHECKPOINT','metadata.editing');
   const next=compileAssembly(data.blueprint);
   if(!sameData(checkpoint.configuration,next.configuration)||!sameData(data.mapping,next.mapping)||!sameData(data.connections,next.connections))reject('INVALID_CHECKPOINT','metadata');
  }catch{reject('INVALID_CHECKPOINT','metadata');}
  session.restore(checkpoint);
  past.length=0;future.length=0;
  session.setMetadata({...checkpoint.metadata,editing:{undoCount:0,redoCount:0}});
  return session.observe();
 }
 return Object.freeze({act,observe:session.observe,
  step(n=1){available();return session.step(n);},
  advanceTime(ms){available();return session.advanceTime(ms);},
  runUntil(predicate,n){available();return session.runUntil(predicate,n);},
  checkpoint(){available();return session.checkpoint();},restore,
  save(){return immutableCopy(metadata().blueprint);},
