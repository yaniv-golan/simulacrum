...
const RESOURCE_BEHAVIORS = new Set(["material-resource"]);
    ...
    descriptor = {
      ...
      aerothermal: {
        ...
        noseRadiusM: Math.max(
          0.025,
          Number(
            config.noseRadius || Math.min(dimensions[0], dimensions[2]) / 2,
          ),
        ),
      },
      provenance: {
...
export function posePartForPortMatch({
  ...
  return deepFreeze({
    positionM: targetWorld.map((value, axis) => value - movingOffset[axis]),
