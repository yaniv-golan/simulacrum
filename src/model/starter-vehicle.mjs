...
export function createStarterVehicle({ powered = true } = {}) {
  ...
  blueprint.parts = [
    ...
    ['powerCell', 'cell'],
    ['gripWheel', 'drive'],
    ['passiveBearing', 'front-bearing'],
    ['gripWheel', 'front-wheel'],
    ['passiveBearing', 'rear-bearing'],
    ['gripWheel', 'rear-wheel'],
  ].map(([type, id]) => createPart(type, id, [0, 0.105, 0]));
  // Authored motor rating supplies the torque needed by the loaded wheel contacts.
  blueprint.parts[1].parameters.torqueConstant = 0.4;
  const names = [
    'Chassis',
    'Motor',
    'Cell',
  ...
  function connect(a, b, kind) {
    if (kind !== 'power') blueprint = snapConnection(blueprint, a, b);
  ...
  connect(face('frame', 'right'), face('motor', 'left'), 'fixed');
  connect({ part: 'motor', port: 'shaft' }, { part: 'drive', port: 'axle' }, 'shaft');
  connect(face('frame', 'top'), face('cell', 'bottom'), 'fixed');
  connect(face('frame', 'left', 0, -0.18), face('front-bearing', 'left'), 'fixed');
  connect({ part: 'front-bearing', port: 'shaft' }, { part: 'front-wheel', port: 'axle' }, 'shaft');
  connect(face('frame', 'left', 0, 0.18), face('rear-bearing', 'left'), 'fixed');
  connect({ part: 'rear-bearing', port: 'shaft' }, { part: 'rear-wheel', port: 'axle' }, 'shaft');
