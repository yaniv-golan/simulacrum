...
try {
  ...
  assert.equal(await page.locator('.inspection-banner').isVisible(), true);
  assert.deepEqual((await read()).frame, before.frame, 'inspection cannot alter telemetry');
  assert.notDeepEqual(
    (await read()).poses,
    before.poses,
  ...
  assert.match(await page.locator('.trace-description').textContent(), /transmits rotation/);
  ...
  assert.ok(trace.context.ui.explodedView.active);
  assert.equal(trace.context.ui.explodedView.amount, 1);
  assert.ok(
    trace.context.ui.explodedView.displayOffsets.some((p) => p.offset.some((v) => v !== 0)),
  );
  assert.equal(trace.context.ui.explodedView.tracedConnection, trace.data.id);
  await page.keyboard.press('ArrowRight');
  assert.deepEqual((await read()).frame, before.frame, 'inspection arrows do not edit');
  await page.keyboard.press('Escape');
  assert.equal(await page.locator('.part-list-item.selected').count(), 0);
  assert.equal(await page.locator('.inspection-banner').isVisible(), true);
  ...
  assert.equal(await page.locator('.inspection-banner').isVisible(), false);
  await page.locator('[data-command=explode-view]').click();
  await page.waitForTimeout(600);
  await page.locator('[data-command=run]').click();
  await page.waitForTimeout(250);
  assert.equal(await page.locator('.inspection-banner').isVisible(), false);
  assert.ok((await read()).frame.tick > 0);
  await page.locator('[data-command=pause]').click();
  await page.locator('[data-command=build]').click();
  assert.deepEqual((await read()).frame.metadata.blueprint, before.frame.metadata.blueprint);
