...
try {
  ...
  for (let i = 0; i < 16; i++) {
    ...
    if (i === 7) {
      ...
      assert.ok(label && toolbar, 'guided selection exposes its label and toolbar');
      assert.ok(
  ...
  assert.equal(await page.locator('.part-settings').evaluate((el) => el.open), false);
  assert.equal(await page.locator('.part-settings > summary').textContent(), 'Engineering details');
  const compact = await withinInspector(
    page.locator('.port-button,.part-settings > summary,[aria-label="Drive setting"]'),
  );
  assert.equal(await page.locator('.port-button').count(), 3);
  ...
  assert.equal(await page.getByRole('button', { name: 'Detach', exact: true }).isVisible(), true);
  assert.ok(
  ...
  assert.equal(await strength.isVisible(), true);
  await strength.press('Home');
  await strength.press('ArrowRight');
  assert.equal((await read()).parts.find((p) => p.name === 'Motor').parameters.defaultDuty, -0.95);
  await page
    .locator('.drive-directions')
    .getByRole('button', { name: 'Reverse', exact: true })
    .click();
  assert.equal((await read()).parts.find((p) => p.name === 'Motor').parameters.defaultDuty, -1);
  await page
    .locator('.drive-directions')
  ...
  assert.deepEqual(await read(), before, 'opening an occupied power port only inspects');
  await page.getByRole('button', { name: 'Cancel connection', exact: true }).click();
  assert.equal(await page.locator('.port-explanation').count(), 0);
  assert.ok(
  ...
  for (const port of await page.locator('.port-button').all()) {
    assert.equal(await port.isEnabled(), true, 'paused ports remain inspectable');
  }
  await page.locator('.port-button[data-port-id=power]').click();
  assert.equal(await page.locator('.port-explanation [data-disconnect-id]').isEnabled(), false);
  assert.equal(await page.locator('.target-button').count(), 0);
  assert.deepEqual(await read(), paused, 'paused inspection cannot mutate blueprint');
  ...
  assert.equal(await page.locator('.port-button').first().getAttribute('data-port-id'), 'axle');
  assert.equal(
  ...
  assert.match(await page.locator('.signal-ownership').textContent(), /Drive commanded by/);
  assert.equal(await page.locator('.signal-ownership .part-link').textContent(), receiver.name);
  ...
  assert.deepEqual(await page.locator('.port-button.signal .port-title').allTextContents(), [
    'Control input',
    'Control output',
  ]);
  assert.equal(
