...
try {
  ...
  assert.equal(original.type, 'poweredMotor');
  assert.equal(original.position[1], 0.06);
  ...
  assert.deepEqual((await frame()).metadata.blueprint.parts[0].position, original.position);
  await page.mouse.move(px, py);
  await page.mouse.down();
  ...
  assert.equal(await page.locator('[data-edit-tool=rotate]').getAttribute('aria-pressed'), 'true');
  assert.match(await page.locator('.canvas-hint').textContent(), /Drag rings to rotate/);
  await page.keyboard.press('v');
  assert.equal(await page.locator('[data-edit-tool=select]').getAttribute('aria-pressed'), 'true');
  ...
  assert.deepEqual(rotated.position, moved.position);
  assert.ok(Math.abs(Math.abs(rotated.rotation[3]) - Math.SQRT1_2) < 1e-9);
  await page.keyboard.press('c');
  let bp = (await frame()).metadata.blueprint;
  assert.equal(bp.parts.length, 2);
  assert.equal(bp.connections.length, 0);
  assert.equal(bp.parts[1].type, rotated.type);
  assert.deepEqual(bp.parts[1].rotation, rotated.rotation);
  assert.ok(
    Math.abs(Math.hypot(...bp.parts[1].position.map((v, i) => v - rotated.position[i])) - 1) < 1e-9,
  );
  await page.keyboard.press('Delete');
  assert.equal((await frame()).metadata.blueprint.parts.length, 1);
  await page.keyboard.press('ControlOrMeta+z');
  assert.equal((await frame()).metadata.blueprint.parts.length, 2);
  if (!(await page.locator('.machine-picker').evaluate((el) => el.open)))
    await page.locator('.machine-picker > summary').click();
  await page.locator('.part-list-item').first().click();
  await page.keyboard.press('ControlOrMeta+c');
  assert.equal((await frame()).metadata.blueprint.parts.length, 3);
