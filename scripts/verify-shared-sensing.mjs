...
try {
  ...
  for (const kind of ['contact', 'tilt', 'jointAngle', 'linearMotion', 'contactLoad']) {
    ...
    if (kind === 'contact') {
      ...
      // narrow contact pad away from the obstacle. Pause the browser clock before
      // arming, then advance one normal tick.
      ...
      // rendered frame. Keep the original 20-second wall-time budget and cap
      // progression at 2400 normal ticks (20 simulated seconds).
