...
try {
  ...
  await attempt('inserted-selection-shortcuts', async (p, load, observed) => {
    ...
    await p.waitForFunction(
      (count) => JSON.parse(window.render_game_to_text()).metadata.blueprint.parts.length === count,
  ...
  await attempt('browse-place-cancel', async (p, load, observed) => {
    ...
    evidence.assert('ok', [Math.abs(Number(await x.inputValue()) - Number(initial) - 0.1) < 1e-9]);
    await placement.getByRole('combobox', { name: 'Rotation axis', exact: true }).selectOption('Y');
