...
try {
  ...
  await page.locator('[data-command=run]').click();
  await page.keyboard.down('KeyW');
  // 480 driven ticks from the tick the page saw the key, then read while it is still held:
  // release is off and the load can fall, so a late pause click would measure harness latency
  // instead, and a key that reached a slow page late would have driven fewer ticks.
  await drivenTicks(page, 480, { label: 'repaired KeyW' });
  ...
  await page.locator('[data-command=run]').click();
  await page.keyboard.down('KeyW');
  await drivenTicks(page, 240, { label: 'gate KeyW' });
  await page.keyboard.up('KeyW');
  await page.locator('[data-command=pause]').click();
