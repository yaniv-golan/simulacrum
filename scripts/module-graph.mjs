...
export function buildModuleGraph(root=process.cwd(), {dataDependencies={},entrypoints,purpose='resources'}={}) {
 ...
 let contract={};
 try {if(existsSync(resolve(root,'package.json')))contract=JSON.parse(readFileSync(resolve(root,'package.json'),'utf8')).moduleGraph??{};}
 catch(error){errors.push(`package.json: invalid module graph contract: ${error.message}`);}
 const record=value=>value!==null&&typeof value==='object'&&!Array.isArray(value);
 if(!record(contract)||Object.keys(contract).some(key=>!['runtimeNetworkDependencies','runtimeURLArguments'].includes(key))) {
  errors.push('package.json: invalid moduleGraph');contract={};
 }
 const network={},urlArguments={};
 const expressions=value=>Array.isArray(value)&&value.every(x=>typeof x==='string'&&/^[A-Za-z_$][\w$]*(?:\.[A-Za-z_$][\w$]*)*$/.test(x));
 for(const [key,destination] of [['runtimeNetworkDependencies',network],['runtimeURLArguments',urlArguments]]) {
  if(contract[key]!==undefined&&!record(contract[key])){errors.push(`package.json: invalid ${key}`);continue;}
  for(const [path,value] of Object.entries(contract[key]??{})) {
   const valid=key==='runtimeURLArguments'
    ? (path.startsWith('scripts/')||Object.hasOwn(network,path))&&expressions(value)
    : path.startsWith('src/application/')&&record(value)&&Object.keys(value).every(x=>['namespace','service','dynamicFetchArguments'].includes(x))&&
      typeof value.namespace==='string'&&/^\/api\/(?:[A-Za-z0-9_-]+\/)+$/.test(value.namespace)&&
      typeof value.service==='string'&&value.service.startsWith('.')&&expressions(value.dynamicFetchArguments);
   if(!files.includes(path)||!valid){errors.push(`${path}: invalid ${key} contract`);continue;}
   destination[path]=value;
  }
 }

 for(const path of files) if(lstatSync(resolve(root,path)).isSymbolicLink()) errors.push(`${path}: symlink dependencies are unsupported`);
 const pending=entrypoints ? [...entrypoints] : files.filter(path=>sourceExtensions.has(extname(path)));
 function targetFor(path,specifier) {
   if(!specifier.startsWith('.')&&!specifier.startsWith('/')) return null;
   const target=relative(root,resolve(root,specifier.startsWith('/')?'.':dirname(path),specifier.split(/[?#]/)[0].replace(/^\//,''))).replaceAll('\\','/');
   if(target.startsWith('../')) {errors.push(`${path}: dependency escapes project: ${specifier}`);return null;}
   if(!existsSync(resolve(root,target)) || !statSync(resolve(root,target)).isFile()) {errors.push(`${path}: unresolved dependency ${specifier}`);return null;}
   if(realpathSync(resolve(root,target))!==resolve(root,target)) {errors.push(`${path}: symlink dependency ${specifier}`);return null;}
 ...
 for(let i=0;i<pending.length;i++) {
   ...
   if(runtime) {
    ...
    if(!service.startsWith('scripts/')||!sourceExtensions.has(extname(service)))errors.push(`${path}: runtime service must be a project script`);
    else add(runtime.service,'runtime-service');
   }
   const scripts=[];
   const css=(text,context='stylesheet')=>{
     try {const tree=cssTree.parse(text,{context,onParseError:error=>{throw error;}});cssTree.walk(tree,node=>{
     ...
     });}catch(error){errors.push(`${path}: CSS parse error: ${error.message}`);}
   };
   if(sourceExtensions.has(extname(path)))scripts.push(readFileSync(resolve(root,path),'utf8'));
     ...
     const visit=node=>{
       ...
       if(node.tagName==='base'&&attrs.href!==undefined)errors.push(`${path}: HTML base href is unsupported`);
       if(attrs.srcset!==undefined)errors.push(`${path}: srcset requires an explicit supported resource manifest`);
       for(const name of ['src','href','poster','data'])if(attrs[name]!==undefined)resource(attrs[name]);
       ...
       if(node.tagName==='script') {
         ...
         else if(!attrs.src && (!attrs.type||['module','text/javascript','application/javascript'].includes(attrs.type)))scripts.push((node.childNodes??[]).map(child=>child.value??'').join(''));
       }
       for(const [name,value] of Object.entries(attrs))if(name.startsWith('on'))scripts.push(value);
   ...
   catch(error){errors.push(`${path}: parse error: ${error.message}`);continue;}
   walk(ast,node=>{
     if(['ImportDeclaration','ExportNamedDeclaration','ExportAllDeclaration'].includes(node.type)&&node.source) add(node.source.value,'module');
     ...
     if((node.type==='NewExpression'&&node.callee.name==='URL')||(node.type==='CallExpression'&&node.callee.name==='fetch')) {
       ...
       if(node.type==='CallExpression'&&runtime) {
        if(value?.type==='Literal'&&typeof value.value==='string'&&value.value.startsWith(runtime.namespace)) {
         ...
         if(!/[\\%]/.test(pathname)&&!pathname.split('/').some(x=>x==='.'||x==='..'))return;
        } else if(value?.type!=='Literal'&&runtime.dynamicFetchArguments.includes(argument))return;
       }
       if(node.type==='NewExpression'&&urlArguments[path]?.includes(argument))return;
       if(value?.type==='Literal'&&typeof value.value==='string') {
         if(!/^[a-zA-Z][a-zA-Z+.-]*:/.test(value.value)) add(value.value.startsWith('.')||value.value.startsWith('/')?value.value:`./${value.value}`,'data');
       } else if(!(dataDependencies[path]?.length)) errors.push(`${path}: nonliteral resource reference requires declared dataDependencies`);
     }
     if(node.type==='NewExpression'&&node.callee.name==='XMLHttpRequest'&&!(dataDependencies[path]?.length)) errors.push(`${path}: XMLHttpRequest requires declared dataDependencies`);
     if(node.type==='Identifier'&&['window','document','HTMLElement','HTMLCanvasElement','customElements','localStorage','sessionStorage'].includes(node.name)) info.dom=true;
     if(node.type==='Literal'&&['window','document','HTMLElement','customElements','localStorage','sessionStorage'].includes(node.value))info.dom=true;
   });
   }
 }
 for(const path of files.filter(path=>path.startsWith('src/')&&!sourceExtensions.has(extname(path))&&!['.json','.html','.htm','.css','.svg','.png','.jpg','.wasm'].includes(extname(path)))) errors.push(`${path}: unsupported source format`);
...
export function validateLayers(graph,{physicsPackages=[]}={}) {
 ...
 for(const [path,node] of graph.nodes) {
  ...
  const owner=layer(path);if(!allowed[owner]){errors.push(`${path}: unknown layer`);continue;}
  for(const edge of node.imports) {
   if(edge.kind==='runtime-service')continue;
   if(path.startsWith('src/simulation/physics/law/')){errors.push(`${path}: law imports nothing (${edge.specifier})`);continue;}
   if(edge.target){const targetLayer=layer(edge.target);if(!targetLayer || (targetLayer!==owner&&!allowed[owner].includes(targetLayer)))errors.push(`${path}: forbidden dependency ${edge.target}`);}
   else if(physicsPackages.some(name=>edge.specifier===name||edge.specifier.startsWith(`${name}/`))){if(!path.startsWith('src/simulation/physics/'))errors.push(`${path}: physics package outside physics boundary`);}
   else if(owner!=='presentation'&&owner!=='application')errors.push(`${path}: undeclared external dependency ${edge.specifier}`);
  }
 }
 const active=new Set(),done=new Set();
 function cycle(path){if(active.has(path)){errors.push(`${path}: module cycle`);return;}if(done.has(path))return;active.add(path);for(const dependency of graph.nodes.get(path)?.dependencies??[])cycle(dependency);active.delete(path);done.add(path);}
 for(const path of graph.nodes.keys())if(path.startsWith('src/'))cycle(path);
 const reachable=new Set();function checkDOM(path){if(reachable.has(path))return;reachable.add(path);const node=graph.nodes.get(path);if(node?.dom)errors.push(`${path}: DOM use reachable from core`);for(const dependency of node?.dependencies??[])checkDOM(dependency);}
 for(const path of graph.nodes.keys())if(path.startsWith('src/core/'))checkDOM(path);
 return [...new Set(errors)];
}
export function checkLayers(root=process.cwd(),options={}) {const errors=validateLayers(buildModuleGraph(root),options);if(errors.length)throw new Error(errors.join('\n'));}
