...
try{
 ...
 await browserEvidence.reload(page);await page.waitForFunction(()=>window.workshopProbe);await page.locator('.recording-panel summary').click();const second=page.waitForEvent('download');await page.locator('[data-command=export-session]').click();assert.deepEqual(JSON.parse(readFileSync(await (await second).path(),'utf8')),capture);
 assert.deepEqual(errors,[]);browserEvidence.assertUnchanged();writeFileSync('artifacts/recording-browser/result.json',JSON.stringify({...browserEvidence.identity,build:capture.build,events:capture.events.length,errors,checks:['stationary guide action across all steps','opt-in only','keys and command results','selection and tools','checkpoint and cursor','export survives reload']},null,2));console.log('guide and local recording browser passed');
