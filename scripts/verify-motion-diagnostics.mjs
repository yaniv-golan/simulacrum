...
try {
  ...
  assert.match(await page.locator('.port-explanation').textContent(), /Carries electrical power/);
  ...
  await page.locator('[data-command=check-machine]').click();
  assert.equal(await dialog.locator('[data-diagnostic-code]').count(), 0);
  assert.match(await dialog.textContent(), /Run the machine to test/);
  ...
  await page.locator('[data-command=check-machine]').click();
  assert.equal(await dialog.locator('[data-diagnostic-code=COMMAND_OFF]').count(), 1);
  ...
  assert.equal(build, appFingerprint());
