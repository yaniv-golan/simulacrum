...
/**
 * Machine-generated modules skip alias/origin analysis, which is quadratic in
 * declarator count and pathological on single-expression generator output.
 *
 * The exemption is keyed on PROVENANCE, never on path. A path-only rule makes
 * `src/model/generated/` a policy blind spot: that directory is already
 * eslint-ignored and capability-dispatch-exempt, and the source-text rules in
 * verify-architecture.mjs only scan `src/simulation`, so a hand-written module
 * placed there would escape every static gate in the repository. Requiring the
 * generator's own banner means an exempt module is one that
 * `npm run check:generated` reproduces byte-for-byte from the schema family.
 *
 * Imports are still collected for exempt modules, so layer direction, missing
 * imports and cycle detection are unaffected.
 */
const GENERATED_SOURCE_BANNER =
  "// Generated by scripts/generate-wire-validators.mjs. Do not edit.";

function isGeneratedModuleSource(source) {
  return source.startsWith(GENERATED_SOURCE_BANNER);
}
...
export async function analyzeArchitecture({
  ...
  for (const file of files) {
    const source = await fs.readFile(file, "utf8"),
      generated = isGeneratedModuleSource(source);
