...
for (const [fixtureName, selectAssembly] of [
  ["blueprint.spec.json", (value) => value],
  ["workspace.spec.json", (value) => value.blueprint],
  ["subassembly.spec.json", (value) => value],
  ["share-package.spec.json", (value) => value.asset],
]) {
  const fixturePath = path.join(
      root,
      "test/fixtures/strict-current-contract",
      fixtureName,
    ),
    fixture = JSON.parse(await fs.readFile(fixturePath, "utf8"));
  scanAssembly(
    `strict-current-contract/${fixtureName}`,
    selectAssembly(fixture),
  );
}
