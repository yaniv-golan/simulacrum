...
try{
 ...
 await page.addInitScript(()=>{
  ...
  window.captureRequests=0;navigator.mediaDevices.getDisplayMedia=async()=>{window.captureRequests++;return canvas.captureStream();};navigator.mediaDevices.getUserMedia=async()=>canvas.captureStream();
  window.pendingRecorders=[];window.stoppedTracks=0;
  window.MediaRecorder=class{static isTypeSupported(){return true;}constructor(stream){this.stream=stream;this.state='inactive';}start(){this.state='recording';}stop(){this.state='inactive';window.pendingRecorders.push(this);}};
  window.flushRecorders=()=>{for(const r of window.pendingRecorders.splice(0)){r.ondataavailable({data:new Blob(['final media'],{type:'video/webm'})});r.onstop();}};
 });
 let sequence=0,held=null,holdComment=true,badReceipt=false,statusCode=200,sessionPosts=0;const uploads=[];
 await page.route('**/api/playtest/**',async route=>{
  const path=new URL(route.request().url()).pathname;
  if(path.endsWith('/config'))return route.fulfill({json:{enabled:true}});
  if(path.endsWith('/session')){sessionPosts++;return route.fulfill({json:{sessionId:'test-session'}});}
  uploads.push({url:route.request().url(),body:route.request().postData()});
  if(path.endsWith('/event')&&route.request().postDataJSON().kind==='feedback-text'&&holdComment){held=route;return;}
 ...
 await page.waitForTimeout(1200);assert.deepEqual(await geometry(),initial,'automatic state uploads cannot move feedback controls');
 await page.getByRole('button',{name:'Give feedback',exact:true}).click();
 await page.getByRole('button',{name:'Close feedback',exact:true}).waitFor({timeout:3000});
 await page.getByRole('textbox',{name:'Your feedback'}).fill('Keep this exact comment visible.');
 await page.getByRole('button',{name:'Send written feedback'}).click();
 await page.waitForFunction(()=>document.querySelector('.playtest-comment')?.textContent.includes('Sending to Yaniv'));
 assert.match(await page.locator('.playtest-comment').innerText(),/Keep this exact comment visible/);
 assert.doesNotMatch(await page.locator('.playtest-comment').innerText(),/Received by/);assert.deepEqual(await geometry(),initial,'pending comment cannot resize the bar');assert.doesNotMatch(await page.locator('[data-status]').textContent(),/uploads pending/);
 while(!held)await new Promise(r=>setTimeout(r,10));
 await held.fulfill({json:{}});holdComment=false;badReceipt=true;
 await page.waitForFunction(()=>document.querySelector('.playtest-comment')?.textContent.includes('Not received yet'));
 assert.doesNotMatch(await page.locator('.playtest-comment').innerText(),/Received by/);
 assert.deepEqual(await geometry(),initial,'retry message cannot move controls');badReceipt=false;
 await page.waitForFunction(()=>document.querySelector('.playtest-comment')?.textContent.includes('Received by Yaniv'));
 for(const [code,message] of [[401,'Open your invitation again'],[403,'Open your invitation again'],[404,'session is unavailable'],[413,'upload is too large'],[503,'retrying automatically']]){
  statusCode=code;
  await page.getByRole('textbox',{name:'Your feedback'}).fill(`Failure ${code}`);
  await page.getByRole('button',{name:'Send written feedback'}).click();
  await page.waitForFunction(text=>document.querySelector('[data-status-detail]').textContent.includes(text),message);
  assert.doesNotMatch(await page.locator('.playtest-comment').last().innerText(),/Received by/);
  statusCode=200;
  await page.waitForFunction(()=>[...document.querySelectorAll('.playtest-comment')].at(-1).textContent.includes('Received by Yaniv'));
 }
 await page.evaluate(()=>{window.hangNext=true;});
 ...
 for(const close of ['x','escape']){
  ...
  if(close==='x')await page.getByRole('button',{name:'Close feedback',exact:true}).click();else await page.keyboard.press('Escape');
  assert.equal(await page.evaluate(()=>window.pendingRecorders.length),1);
  assert.equal(await page.evaluate(()=>window.pendingRecorders[0].stream.getTracks().every(t=>t.readyState==='ended')),true);
 ...
 assert.doesNotMatch(await page.locator('[data-completion-status]').innerText(),/You can close this tab/);
 await page.waitForTimeout(150);
 assert.doesNotMatch(await page.locator('[data-completion-status]').innerText(),/You can close this tab/);
 await page.evaluate(()=>window.flushRecorders());
 await page.waitForFunction(()=>document.querySelector('[data-completion-status]').textContent==='Feedback received — session saved. You can close this tab.');
 await page.evaluate(()=>{window.originalTransaction=IDBDatabase.prototype.transaction;IDBDatabase.prototype.transaction=function(...args){if(args[1]==='readonly')throw Error('outbox unavailable');return window.originalTransaction.apply(this,args);};});
 await page.waitForFunction(()=>document.querySelector('[data-completion-status]').textContent.includes('Session not fully saved'));
 assert.doesNotMatch(await page.locator('[data-completion-status]').innerText(),/You can close this tab/);
 ...
 assert.equal(sessionPosts,sessionsBeforeReload,'reload never silently creates a capture session');
 assert.equal(await page.getByRole('button',{name:'Give feedback',exact:true}).isDisabled(),true);
 assert.match(await page.locator('[data-recovery]').innerText(),/saved uploads from an earlier session/i);
 ...
 statusCode=200;badReceipt=true;
 const beforeMalformed=uploads.length;
 for(let attempt=0;uploads.length===beforeMalformed&&attempt<100;attempt++)await new Promise(resolve=>setTimeout(resolve,50));
 assert.ok(uploads.length>beforeMalformed,'saved uploads retry after reload');
 await page.waitForTimeout(100);
 assert.ok((await outbox()).some(row=>row.id===savedBody.id),'malformed ACK after reload cannot delete saved feedback');
 ...
 assert.ok(uploads.slice(uploadsBeforeReload).some(upload=>upload.url.endsWith(savedBody.url)&&upload.body===savedBody.body),'exact persisted feedback body retried to its original session');
