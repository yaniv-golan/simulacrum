...
export const CHECKS = Object.freeze({
  'no-live-state-reads': () =>
    node(['--test', 'test/controllers.test.mjs', 'test/m3-session.test.mjs']),
  'identity-invariance-minimal': async () => {
    await node(['--test', 'test/m3-authoring.test.mjs']);
    return node(['scripts/verify-m3.mjs']);
  },
  'workshop-response': () =>
    node(
      ['scripts/verify-browser-suite.mjs', 'performance', '--reuse-build'],
      suiteDeadline('performance'),
    ),
  'production-control-path': async () => {
    await node(['--test', 'test/free-build-continuity.test.mjs']);
    return node(
      ['scripts/verify-browser-suite.mjs', 'smoke', '--reuse-build'],
      suiteDeadline('smoke'),
    );
  },
  ...
  'schema-rejects-malformed': async () => {
    const { generateSchema } = await import('./generate-schema.mjs');
    generateSchema({ check: true });
    return node(['--test', 'test/blueprint.test.mjs']);
  },
  'single-library-importer': async () => {
    const { checkPhysicsBoundary } = await import('./check-physics-boundary.mjs');
    checkPhysicsBoundary();
  },
  'no-live-object-escape': () =>
    node(['--test', 'test/physics.test.mjs', 'test/workshop.test.mjs']),
  'milestone-breadth-reject': async () => {
