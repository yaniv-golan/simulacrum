import { withCleanup } from './verification-cleanup.mjs';
  ...
  async () => {
    try {
      ...
      if (
        ...
        browserEvidence.assert('equal', [
          await page
            .getByRole('button', { name: 'Replace without saving', exact: true })
            .isVisible(),
          false,
        ...
        try {
          ...
          browserEvidence.assert('ok', [
            retryFault.state.injected > 0,
            'a real upload was attempted',
          ]);
          ...
          browserEvidence.assert('equal', [
            (await page.evaluate(() => window.workshopProbe.readLastCommandResult())).result
              .reasonCode,
            'RETRY_PENDING',
      ...
      if (faultName === 'reload-recovery') {
        await page
          ...
          .waitFor();
        browserEvidence.assert('ok', [
          (await page.locator('.playtest-panel [data-status]').innerText()).includes(
            'Ready to record',
          ),
      ...
      const id = readdirSync(data)[0],
        dir = join(data, id),
        records = readFileSync(join(dir, 'events.ndjson'), 'utf8')
          .trim()
          .split('\n')
          .map(JSON.parse),
      ...
      if (retryEpochs) {
        ...
        browserEvidence.assert('ok', [
          contexts.some(
            (e) =>
              e.kind === 'command-result' && e.data.input?.type === 'retry' && e.data.result.ok,
          ),
          'actual application retry receipt retained',
        ]);
        browserEvidence.assert('ok', [
          contexts.every(
            (e) => e.context?.observation?.metadata?.blueprint?.id !== 'wrong-document',
          ),
        ...
        writeFileSync(
          ...
          JSON.stringify(
            await sampleCapture({
              ...
              mediaFiles: records
                .filter(
                  (row) => row.media?.kind === (recordingMode === 'data' ? 'voice' : 'screen'),
                )
