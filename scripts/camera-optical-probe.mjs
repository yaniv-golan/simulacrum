...
export async function cameraOpticalProbe() {
  ...
  const centerPixel = () =>
    Array.from(image().slice(4 * (120 * 320 + 160), 4 * (120 * 320 + 160) + 3));
  const ropeBackground = centerPixel();
  frame.ropes = [
    {
      diameter: 0.02,
      points: [
  ...
  const ropePixel = centerPixel();
  check(
    ropePixel.some((v, i) => v !== ropeBackground[i]),
    'completed rope across lens must occlude sky pixels',
  );
  ...
  check(
    centerPixel().every((v, i) => v === ropeBackground[i]),
    'old rope pose must leave optical pixels',
  );
  delete frame.ropes;
  frame.physics[1] = body([0.2, 2, 1]);
  const shape = {
  ...
  try {
    ...
    return {
      ...
      liveHandles,
      projected,
      ropeOptics: { background: ropeBackground, rope: ropePixel, movedClear: true },
      before,
      turned,
      timings,
      resources,
