...
} from '../src/application/interaction-recorder.mjs';
...
function fixture(options = {}) {
  ...
  const config = {
    ...
    storage,
    now: () => time,
    wallNow: () => '2026-09-05T12:00:00.000Z',
    idFactory: () => 'session-1',
    ...options,
  };
  return {
...
test('capture is explicitly started and stopped; elapsed clock never moves backwards', () => {
  ...
  f.time(120);
  assert.equal(r.record('key', { key: 'w' }, { tick: 1 }), true);
  f.time(110);
  assert.equal(r.record('command', { command: 'run' }), true);
  assert.deepEqual(
    r.snapshot().events.map((e) => [e.seq, e.timeMs]),
    [
      [1, 20],
      [2, 20],
    ],
...
test('storage errors stop capture and leave all accepted events available in memory', () => {
  ...
  assert.equal(r.state().reason, 'storage-failure');
  assert.equal(r.state().persisted, false);
  assert.equal(r.snapshot().events.length, 1);
  assert.match(r.state().error, /quota/);
  assert.equal(r.record('key', {}), false);
});

test('stored capture survives reload and all inputs and outputs are isolated copies', () => {
  const f = fixture();
  const context = { nested: { x: 1 } };
  const data = { list: [1] };
  f.recorder.start(context);
  f.recorder.record('pointer', data, context);
  f.recorder.stop(context);
  context.nested.x = 9;
  data.list.push(2);
  const snapshot = f.recorder.snapshot();
  snapshot.events[0].data.list.push(3);
  const reloaded = createInteractionRecorder(f.config);
  const previous = reloaded.readLast();
...
test('malformed and foreign stored documents are refused', () => {
  ...
  for (const bad of ['{', '{}', 'null', JSON.stringify({ version: 1, events: [] })]) {
    f.entries.set(INTERACTION_STORAGE_KEY, bad);
...
test('invalid JSON values stop recording without corrupting the complete prefix', () => {
  for (const data of [{ n: NaN }, { f() {} }, { a: undefined }, { n: Infinity }, { n: 1n }]) {
    const { recorder: r } = fixture();
    r.start();
    r.record('good', {});
    assert.equal(r.record('bad', data), false);
...
test('oversized initial and final contexts cannot evade the byte bound', () => {
  ...
  assert.equal(r.start({ huge: 'x'.repeat(2000) }), false);
  assert.equal(r.snapshot(), null);
  assert.equal(r.start(), true);
  assert.equal(r.stop({ huge: 'x'.repeat(2000) }), false);
  assert.equal(r.state().reason, 'byte-limit');
  assert.ok(Buffer.byteLength(JSON.stringify(r.snapshot())) <= 700);
});
...
test('byte budget accepts an event exactly at the reserved terminal boundary', () => {
  ...
  sample.recorder.start();
  sample.recorder.record('key', { key: 'w' });
  const expected = {
    ...sample.recorder.snapshot(),
    endedAt: '2026-09-05T12:00:00.000Z',
    status: 'stopped',
    terminationReason: 'storage-failure',
  };
  const boundary = Buffer.byteLength(JSON.stringify(expected));
  const exact = fixture({ maxBytes: boundary });
  exact.recorder.start();
  assert.equal(exact.recorder.record('key', { key: 'w' }), true);
  const under = fixture({ maxBytes: boundary - 1 });
  under.recorder.start();
