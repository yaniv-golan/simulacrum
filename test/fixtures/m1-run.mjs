...
export const inputTrace = [
  { tick: 10, command: { type: 'impulse', body: 0, value: [0.25, 0, 0] } },
  { tick: 60, command: { type: 'impulse', body: 0, value: [0, 0.5, 0] } },
];
export function digest(value) {
  return createHash('sha256')
    .update(ArrayBuffer.isView(value) ? value : JSON.stringify(value))
    .digest('hex');
}
export async function runFixture(driver) {
  ...
  try {
    for (let tick = 1; tick <= 120; tick++) {
      for (const input of inputTrace.filter((input) => input.tick === tick)) {
        const accepted = session.act(input.command);
        if (!accepted.ok) throw Error(`input rejected at tick ${tick}: ${accepted.reasonCode}`);
      }
      ...
      if (frame.tick !== tick)
        throw Error(`clock tick mismatch: expected ${tick}, got ${frame.tick}`);
