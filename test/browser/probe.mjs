...
function render() {
  ...
  if (result.ok) {
    for (const frame of result.frames) trace.push(deterministicProjection(frame));
  ...
  ctx.strokeStyle = '#b5c5d1';
  ctx.fillStyle = '#536675';
  ...
  for (let y = 0; y <= 12; y += 2) {
    ...
    ctx.lineTo(850, py);
    ctx.stroke();
    ctx.fillText(`${y} m`, 8, py + 5);
  }
  ctx.fillStyle = '#167c9e';
  ctx.fillRect(450 + position[0] * 32 - 8, 440 - position[1] * 32 - 8, 16, 16);
  document.querySelector('#state').textContent =
    `Tick ${frame.tick} · height ${position[1].toFixed(3)} m · velocity ${frame.physics[0].velocity[1].toFixed(3)} m/s`;
}
const clock = createClock(session, {
...
window.render_game_to_text = () =>
  JSON.stringify({
    coordinates: 'x right, y up, z depth; meters',
    ...session.observe().frames.at(-1),
  });
