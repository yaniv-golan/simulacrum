...
test('real repository selection is selective and keeps opaque tooling consumers',()=>{
 ...
 assert.ok(selected.includes('test/assessment.test.mjs'),'subprocess/file-driven tooling is conservative');
 assert.ok(!selected.includes('test/power.test.mjs'),'unrelated pure unit suite stays unselected');
 assert.ok(selected.length<affectedTests(graph,undefined).length);
 assert.equal(affectedTests(graph,['unknown.file']).length,affectedTests(graph,undefined).length);
});
