...
test('optical input copies completed rope geometry without connection identity or editor selection', async () => {
  const f = fixture();
  try {
    const o = observation(12, { serial: 1 });
    o.frames[0].metadata.blueprint.connections = [
      { id: 'private-rope', kind: 'rope', rope: { diameter: 0.02 }, selected: true },
    ];
    o.frames[0].metadata.connections = [{ id: 'private-rope', rope: { nodes: [1, 2] } }];
    o.frames[0].physics = [
      { position: [0, 1, 0], rotation: [0, 0, 0, 1] },
      { position: [-1, 1, 1], rotation: [0, 0, 0, 1] },
      { position: [1, 1, 1], rotation: [0, 0, 0, 1] },
    ];
    f.set(o);
    await new Promise((r) => setImmediate(r));
    assert.deepEqual(f.encoded[0].frame.ropes, [
      {
        diameter: 0.02,
        points: [
          [-1, 1, 1],
    ...
    o.frames[0].physics[1].position[0] = 100;
    assert.equal(f.encoded[0].frame.ropes[0].points[0][0], -1);
  } finally {
    f.session.dispose();
  }
});
