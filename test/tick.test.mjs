...
test('reordered phases refuse',()=>assert.throws(()=>fixture((p,s)=>p.endsWith('tick.mjs')?s.replace(/['"]sensor-snapshot['"]\s*,\s*['"]controller-commands['"]/,"'controller-commands', 'sensor-snapshot'"):s),/phase order/));
...
test('reconfiguration may reset clock only to zero',()=>assert.throws(()=>fixture((p,s)=>p.endsWith('session.mjs')?s.replace(/tick\s*=\s*0;\s*accumulator\s*=\s*0;/,'tick=1;accumulator=0;'):s),/clock/));
