  ...
  return JSON.parse(
    execFileSync(
      ...
      [
        '--input-type=module',
        '-e',
        "import config from './vite.config.mjs'; console.log(JSON.stringify(config.cacheDir ?? null))",
      ],
      { env, encoding: 'utf8' },
    ),
  );
...
test('candidate Vite cache redirects only when explicitly configured', () => {
  ...
  assert.equal(
    configuredCache('/tmp/candidate-artifacts/vite-cache'),
    '/tmp/candidate-artifacts/vite-cache',
  );
});
