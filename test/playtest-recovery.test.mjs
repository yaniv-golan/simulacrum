...
async function fixture(t, limits = {}) {
  ...
  const publicDir = join(root, 'public'), dataDir = join(root, 'private');
  await mkdir(publicDir); await writeFile(join(publicDir, 'index.html'), 'ok');
  ...
  const start = async () => {
    ...
    const request = (url, { method = 'GET', headers = {}, body = '' } = {}) => new Promise(resolve => {
      ...
      const res = {
        ...
        end(body) { resolve({ status: this.status, headers: this.headers, body: body && JSON.parse(body) }); },
      };
      server.emit('request', req, res);
    });
    const login = await request(`/join?token=${token}`);
    const cookie = login.headers['set-cookie'].split(';')[0];
    return (path, body, type = 'application/json') => request(path, { method: 'POST', headers: { cookie, 'content-type': type }, body: typeof body === 'string' ? body : JSON.stringify(body) });
  };
  let post = await start();
  const { body: { sessionId } } = await post('/api/playtest/session', {});
  return { dir: join(dataDir, sessionId), path: `/api/playtest/${sessionId}`, post, restart: start };
}
...
test('restart reconciles unacknowledged complete and interrupted media without losing prior receipts', async t => {
  ...
  for (const [seq, bytes] of [[0, 'complete-media'], [1, 'interrupted-media-completed']]) {
...
test('orphan recovery credits existing bytes and ignores uncommitted staging files', async t => {
  ...
  assert.equal((await post(`${f.path}/media?kind=screen&clip=tab&seq=1`, bytes, 'video/webm')).status, 413, 'recovery does not remove the actual storage bound');
