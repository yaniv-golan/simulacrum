...
test('observation has no mutable input or output aliases',()=>{
 const initial=frame(0),store=createObservationStore(initial,{sessionId:'a'});
 ...
 const next=frame(1,2);store.publish(next);next.bodies[0].position.x=3;
...
test('paused edits advance revision and restore starts a new epoch',()=>{
 const store=createObservationStore(frame(0),{sessionId:'a'}),start=store.cursor();
 ...
 store.publish(frame(1));const before=store.cursor();store.publish(frame(0),{restored:true});
...
test('deltas require exact known session epoch revision and tick and bounded window',()=>{
 const store=createObservationStore(frame(0),{sessionId:'a',maxDeltas:2});const zero=store.cursor();
 store.publish(frame(1));const one=store.cursor();store.publish(frame(2));
 assert.deepEqual(store.observe('scene','full',zero).frames.map(x=>x.tick),[1,2]);
 store.publish(frame(3));
 for(const c of [zero,{...one,session:'b'},{...one,revision:9},{...one,tick:9},{tick:1},{...one,revision:1.1},{...one,extra:1}]){
 const response=store.observe('scene','full',c);assert.equal(response.ok,false);assert.equal(response.reasonCode,'RESYNC_REQUIRED');assert.equal(response.frame.tick,3);
 }
 assert.deepEqual(store.observe('scene','full',one).frames.map(x=>x.tick),[2,3]);
});
...
test('invalid publication is atomic and backwards ticks require restore',()=>{
 const store=createObservationStore(frame(2),{sessionId:'a'}),before=store.cursor();
 for(const bad of [frame(1),frame(NaN),{tick:3,fn(){}},{tick:3,date:new Date()}, {tick:3,get unsafe(){throw Error('getter executed')}}])assert.throws(()=>store.publish(bad));
