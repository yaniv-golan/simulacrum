...
test('ordinary workshop commands connect power, shaft and receiver then drive the rotor', async () => {
  ...
  try {
    for (const { id, a, b } of links)
      assert.equal((await workshop.act({ type: 'connect', id, a, b })).ok, true);
...
function connected() {
  ...
  blueprint.connections = structuredClone(links);
  blueprint.parts[3].parameters.duty = 1;
  return blueprint;
}
function renamed(original) {
  const blueprint = structuredClone(original),
    names = new Map();
  blueprint.id = 'different-machine';
  blueprint.name = 'Different visible name';
  blueprint.parts.forEach((part, i) => {
    const id = `renamed-${99 - i}`;
    names.set(part.id, id);
    part.id = id;
    part.name = `Part ${99 - i}`;
  });
  blueprint.connections.forEach((connection, i) => {
    connection.id = `connection-${99 - i}`;
    connection.a.part = names.get(connection.a.part);
    connection.b.part = names.get(connection.b.part);
  });
  return blueprint;
}
async function trace(blueprint, ticks = 60) {
  ...
  try {
    const frames = [deterministicProjection(latest(session))];
    for (let i = 0; i < ticks; i++) {
      session.step(1);
      frames.push(deterministicProjection(latest(session)));
    }
    return frames;
  } finally {
    session.dispose();
...
test('renamed authored identities preserve the complete deterministic projection every tick', async () => {
  const original = connected(),
    other = renamed(original);
  assert.deepEqual(compileAssembly(original).configuration, compileAssembly(other).configuration);
  const first = await trace(original),
    second = await trace(other);
  assert.equal(first.length, 61);
  for (let tick = 0; tick < first.length; tick++)
    assert.deepEqual(first[tick], second[tick], `renaming changed tick ${tick}`);
  ...
  changed.parts[2].authoredMaterial.body = 'steel';
  const wrong = await trace(changed);
  assert.throws(() => assert.deepEqual(first, wrong));
  assert.notDeepEqual(
    first.at(-1).physics[2].angularVelocity,
    wrong.at(-1).physics[2].angularVelocity,
  );
});
